...
const CreateNewGroup = () => {
  ...
  const [privategroups, setprivategroup] = useState([]);
  ...
  useEffect(() => {
    const fetchGroups = async () => {
      try {
        ...
        const publicList = publicSnap.docs.map(doc => doc.data());
        const privateList = privateSnap.docs.map(doc => doc.data());

        setpublicgroup(publicList);
        setprivategroup(privateList);

        console.log("Public groups:", publicList);
        console.log("Private groups:", privateList);
      } catch (err) {
  ...
  const [selectedFriends, setSelectedFriends] = useState<Member[]>([]);
  ...
  const toggleFriendSelection = (friendId: string) => {
    setSelectedFriends(prev => {
      ...
      if (exists) {
        ...
        return [...prev, { id: friendId, role: "member" }];
      }
    });
  };
  const toggleRole = (friendId: string) => {
    setSelectedFriends(prev =>
      prev.map(f =>
        f.id === friendId
...
const CreateGroup = async () => {
  ...
  const allMembers = [
    currentUserMember,
    ...selectedFriends.filter(f => f.id !== user.uid),
  ];
  ...
  const data = {
    ...
    lastmessage: `Welcome everybody to my group: ${groupname}`,
  ...
  try {
    let groupRef;
    if (!isPrivate) {
      const exists = publicgroups.some(
        g => g.groupname.toLowerCase() === groupname.toLowerCase()
      );
      if (exists) {
        alert("A public group with this name already exists!");
        return;
      }
      groupRef = await addDoc(collection(fsdb, "Publicgroups"), data);
    } else {
      groupRef = await addDoc(collection(fsdb, "Privategroups"), data);
    }

    // Create initial Messages subcollection with first welcome message
    await addDoc(collection(fsdb, groupRef.parent.id, groupRef.id, "Messages"), {
      senderId: user.uid,
      textMessage: `Welcome to the group: ${groupname}`,
      timestamp: serverTimestamp()
    });

    console.log("Group created with Messages collection:", data);
    ...
    <section>
                ...
                return (
                  ...
                  >
                    <div className="relative">
                      ...
                      <button
                        onClick={() => toggleFriendSelection(member.id)}
      ...
      <Card className="p-6 shadow-card mt-5 m-4 space-y-6">
        ...
        <div className="space-y-2">
          ...
          <Input placeholder="Group 1" className="max-w-md" onChange={(e) => setgroupname(e.target.value)} />
        </div>
            ...
            <div className="space-y-3">
              {friends.map((friend) => (
                <div
                  ...
                  className={`p-4 rounded-lg cursor-pointer transition-smooth border-2 ${selectedFriends.some(f => f.id === friend.id) ? 'border-primary bg-primary/5'
                    ...
                    <div className="flex items-center gap-2">
                      {selectedFriends.some(f => f.id === friend.id) && (
                        <Button
                          size="sm"
                          variant={
                            selectedFriends.find(f => f.id === friend.id)?.role === "admin"
                              ? "default"
                              : "outline"
                          }
                          onClick={(e) => {
                        ...
                        >
                          {selectedFriends.find(f => f.id === friend.id)?.role === "admin" ? "Admin" : "Member"}
                        </Button>
                      )}
